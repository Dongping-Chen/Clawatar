...
label_mat    = mat("LabelPaper", (0.92, 0.88, 0.78, 1), 0.9)
...
lantern_emit_orange = emit_mat("LanternOrange", (1.0, 0.5, 0.1, 1), 18.0)
lantern_emit_red    = emit_mat("LanternRed",    (1.0, 0.2, 0.05, 1), 17.0)
lantern_emit_warm   = emit_mat("LanternWarm",   (1.0, 0.6, 0.2, 1), 20.0)
...
# ── Back bar counter + drinks ───────────────────────────────────────────
bar_w, bar_d, bar_h = 1.5, 0.45, 0.95
bar_x, bar_y = 0.0, -D/2 + 0.28
rbox("BarBase", (bar_x, bar_y, bar_h/2), (bar_w, bar_d, bar_h), med_wood, r=0.015)
rbox("BarTop", (bar_x, bar_y, bar_h + 0.03), (bar_w+0.06, bar_d+0.06, 0.06), dark_wood, r=0.01)
bar_top_z = bar_h + 0.06

# Sake bottles (back counter)
bottle_specs = [
    ("BottleGreen", -0.5, glass_green),
    ("BottleBrown", -0.2, glass_brown),
    ("BottleClear",  0.1, clear_glass),
    ("BottleGreen2", 0.4, glass_green),
]
for nm, dx, m in bottle_specs:
    bx = bar_x + dx
    by = bar_y - 0.08
    cyl(nm, (bx, by, bar_top_z + 0.14), 0.035, 0.28, m)
    cyl(f"{nm}Neck", (bx, by, bar_top_z + 0.28), 0.018, 0.05, m)
    cyl(f"{nm}Cap", (bx, by, bar_top_z + 0.33), 0.012, 0.02, gold_mat)
    box(f"{nm}Label", (bx, by + 0.041, bar_top_z + 0.13), (0.045, 0.004, 0.10), label_mat)
    if "Clear" in nm:
        cyl(f"{nm}Liquid", (bx, by, bar_top_z + 0.10), 0.028, 0.18, liquid_amber)

# Sake cups/glasses on counter
cup_positions = [(-0.05, 0.14), (0.08, 0.12), (0.22, 0.14)]
for i, (dx, dy) in enumerate(cup_positions):
    cxp = bar_x + dx
    cyp = bar_y + dy
    cup_mat = ceramic_w if i % 2 == 0 else ceramic_blue
    cyl(f"CounterCup{i}", (cxp, cyp, bar_top_z + 0.025), 0.025, 0.05, cup_mat)
    if i == 1:
        cyl(f"CounterCupLiquid{i}", (cxp, cyp, bar_top_z + 0.03), 0.02, 0.03, amber_emit)

# Beer mug with handle
mug_x, mug_y = bar_x + 0.45, bar_y + 0.16
mug_h = 0.11
cyl("BeerMug", (mug_x, mug_y, bar_top_z + mug_h/2), 0.045, mug_h, beer_glass)
cyl("BeerLiquid", (mug_x, mug_y, bar_top_z + mug_h/2 - 0.005), 0.038, mug_h - 0.02, liquid_amber)
torus("BeerHandle", (mug_x + 0.055, mug_y, bar_top_z + 0.045), 0.03, 0.008, beer_glass, rot=(0, math.radians(90), 0))

# Menu stand / chalkboard on counter
menu_x, menu_y = bar_x - 0.6, bar_y + 0.08
rbox("CounterMenuBase", (menu_x, menu_y, bar_top_z + 0.01), (0.14, 0.08, 0.02), med_wood, r=0.004)
rbox("CounterMenuBoard", (menu_x, menu_y - 0.02, bar_top_z + 0.13), (0.18, 0.02, 0.25), menu_board, r=0.004,
     rot=(math.radians(-15), 0, 0))
for i in range(3):
    box(f"CounterChalk{i}", (menu_x + random.uniform(-0.03, 0.03), menu_y - 0.02, bar_top_z + 0.2 - i*0.06),
        (0.08, 0.002, 0.01), chalk_mat, rot=(math.radians(-15), 0, 0))

# Chopsticks & small dish plates
for i, off in enumerate([-0.008, 0.008]):
    cyl(f"Chopstick{i}", (bar_x - 0.35, bar_y + 0.17 + off, bar_top_z + 0.015), 0.004, 0.28, bamboo_mat,
        rot=(0, math.radians(90), math.radians(10)))
for i, (dx, dy, m) in enumerate([(-0.48, 0.12, ceramic_w), (-0.25, 0.18, ceramic_g)]):
    cyl(f"BarDish{i}", (bar_x + dx, bar_y + dy, bar_top_z + 0.008), 0.055, 0.012, m)
