"""
Swimming Pool v8 — ALL EMISSIVE MATERIALS (Three.js compatible)
- Emission strengths >= 3.0 for ALL materials (glTF emissive strength extension)
- Keeps ALL geometry from v6/v7: wavy water, checkerboard deck, palms, fence, etc.
- Night atmosphere with bright accents and visible surfaces
"""
...
def build_pool_area():
    # Deck tiles (checkerboard) — visible, not bright
    m_tile_a = emit_mat("DeckTileA", (0.35, 0.18, 0.06, 1), 4.2)   # terracotta
    m_tile_b = emit_mat("DeckTileB", (0.40, 0.32, 0.22, 1), 3.8)   # sandstone
    ...
    # Pool interior — blue-tinted
    m_pool_wall = emit_mat("PoolTile", (0.10, 0.30, 0.50, 1), 5.0)
    m_pool_floor = emit_mat("PoolTileFloor", (0.08, 0.25, 0.45, 1), 5.0)
    ...
    # Coping / edge
    m_cop = emit_mat("Coping", (0.50, 0.48, 0.42, 1), 3.5)
    ew = 0.15
    ...
    # Pool edge lights
    m_edge = emit_mat("EdgeLight", (0.15, 0.85, 0.95, 1), 15.0)
    light_positions = [
...
def build_loungers():
    m_lounger = emit_mat("Lounger", (0.40, 0.38, 0.35, 1), 4.0)
    m_cushion = emit_mat("LoungerCushion", (0.38, 0.36, 0.34, 1), 4.2)
    m_towel_blue = emit_mat("TowelBlue", (0.25, 0.40, 0.55, 1), 3.8)
    m_table = emit_mat("SideTable", (0.30, 0.28, 0.25, 1), 3.6)
...
def build_umbrella():
    m_pole = emit_mat("UmbrellaPole", (0.30, 0.28, 0.26, 1), 3.5)
    m_seg_a = emit_mat("UmbrellaSegA", (0.45, 0.44, 0.43, 1), 4.0)
    m_seg_b = emit_mat("UmbrellaSegB", (0.12, 0.22, 0.45, 1), 4.0)
    ux, uy = 2.5, -4.0
...
def palm_tree_at(tx, ty):
    m_trunk = emit_mat("PalmTrunk", (0.10, 0.06, 0.03, 1), 3.0)
    m_leaf = emit_mat("PalmLeaf", (0.05, 0.18, 0.08, 1), 3.0)
    m_coconut = emit_mat("Coconut", (0.12, 0.08, 0.04, 1), 3.0)
...
def build_fence():
    m_post = emit_mat("FencePost", (0.18, 0.18, 0.20, 1), 3.0)
    m_glass = emit_mat("FenceGlass", (0.20, 0.25, 0.30, 1), 3.0)
...
def build_deck_lights():
    m_pole = emit_mat("DeckLightPole", (0.10, 0.10, 0.12, 1), 3.0)
...
def build_bar_cart():
    m_wood = emit_mat("CartWood", (0.22, 0.14, 0.08, 1), 3.8)
    m_bottle_g = emit_mat("BottleG", (0.06, 0.28, 0.12, 1), 3.6)
    m_bottle_b = emit_mat("BottleB", (0.08, 0.12, 0.35, 1), 3.6)
    m_glass = emit_mat("GlassCup", (0.40, 0.42, 0.45, 1), 3.5)
...
def build_background():
    # Night sky panel
    m_sky = emit_mat("Sky", (0.02, 0.03, 0.08, 1), 3.0)
    box("Sky", (0, 6.5, 2.5), (STAGE_W + 6, 0.02, 5.0), m_sky)

    # Building silhouette
    m_build = emit_mat("Building", (0.04, 0.04, 0.06, 1), 3.0)
...
def main():
    print("=" * 50)
    print("  Swimming Pool v8 — ALL EMISSIVE MATERIALS")
    print("=" * 50)
    clear_scene()
    build_pool_area()
    build_loungers()
