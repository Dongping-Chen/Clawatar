"""
Café Scene v1 — Reze's Coffee Shop (Chainsaw Man)
Where Reze works and first meets Denji. Small, cozy Japanese café.
Stage concept: counter on back wall, seating front, warm lighting.
Cycles renderer.
"""
...
def build_counter():
    ...
    # ── COFFEE MACHINE ──
    m_mach = mat("Mach", (0.25, 0.25, 0.28, 1), 0.2, 0.6)
    m_mach_a = mat("MachA", (0.90, 0.20, 0.15, 1), 0.3, 0.3)
    rbox("CoffeM", (0.8, -hd+0.15, 1.08), (0.35, 0.25, 0.40), m_mach, 0.02)
    # Red accent strip
    box("CMstrip", (0.8, -hd+0.28, 1.20), (0.33, 0.01, 0.05), m_mach_a)

    # ── CUPS on shelves ──
    m_cup_w = mat("CupW", (0.95, 0.93, 0.90, 1), 0.4)
    m_cup_b = mat("CupB", (0.82, 0.70, 0.55, 1), 0.4)
    for i, (cx, sz) in enumerate([
        (-1.5, 0.57), (-1.2, 0.57), (-0.9, 0.57), (-0.6, 0.57),
        (-1.5, 0.92), (-1.2, 0.92), (-0.9, 0.92),
        (0.3, 1.27), (0.5, 1.27), (0.7, 1.27),
    ]):
        m = m_cup_w if i % 3 != 2 else m_cup_b
        cyl(f"Cup{i}", (cx, -hd+0.12, sz+0.04), 0.025, 0.06, m)

    # ── Items on counter top ──
    # Cake stand / display
    m_glass_d = mat("GlassD", (0.90, 0.92, 0.95, 1), 0.1, 0.05)
    cyl("CakeStand", (-0.5, -hd+0.45, 1.04), 0.12, 0.005, m_glass_d)
    cyl("CakeDome", (-0.5, -hd+0.45, 1.12), 0.11, 0.15, m_glass_d)
...
def build_decorations():
    ...
    m_bulb = emit_mat("Bulb", (1.0, 0.90, 0.65, 1), 12.0)
...
def render():
    ...
    for name, (loc, rot, lens) in angles.items():
        ...
        scene.camera = co
        scene.render.filepath = os.path.join(OUTPUT_DIR, f"cafe_{name}.png")
        bpy.ops.render.render(write_still=True)
        print(f"  ✓ cafe_{name}.png")

    g = os.path.join(OUTPUT_DIR, "cafe-v1.glb")
...
def main():
    print("="*50)
    print("  Café v1 — Reze's Coffee Shop")
    print("="*50)
    clear_scene()
    build_shell()
    build_counter()
