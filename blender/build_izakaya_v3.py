...
# Lantern materials — HERO (20-25)
lantern_emit_orange = emit_mat("LanternOrange", (1.0, 0.5, 0.1, 1), 24.0)
lantern_emit_red    = emit_mat("LanternRed",    (1.0, 0.2, 0.05, 1), 23.0)
lantern_emit_warm   = emit_mat("LanternWarm",   (1.0, 0.6, 0.2, 1), 25.0)
candle_emit         = emit_mat("CandleEmit",    (1.0, 0.65, 0.25, 1), 8.0)

# ── Floor (dark wood plank pattern) ──────────────────────────────────────
box("FloorBase", (cx, cy, 0), (W, D, 0.05), floor_gap)
plank_w = 0.26
gap = 0.02
...
for i in range(count):
    ...
    m = floor_dark if i % 2 == 0 else floor_light
...
for i in range(4):
    x = -W/2 + 0.7 + i * (W/3)
    box(f"CeilBeamX{i}", (x, cy, H-0.06), (0.08, D, 0.12), beam_wood)
for i in range(3):
    y = -D/2 + 0.5 + i * (D/2)
    box(f"CeilBeamY{i}", (cx, y, H-0.06), (W, 0.08, 0.10), beam_wood)
...
# ── Paper lanterns (HERO) ────────────────────────────────────────────────
lantern_data = [
    (tbl_x - 0.45, tbl_y, H - 0.55, lantern_emit_orange, 230, 0.22, 1.5),
    (tbl_x + 0.45, tbl_y, H - 0.55, lantern_emit_red,    220, 0.22, 1.5),
    (tbl_x,        tbl_y, H - 0.40, lantern_emit_warm,    260, 0.24, 1.4),
    (-W/2 + 0.7,   D/2 - 0.7, H - 0.55, lantern_emit_orange, 160, 0.18, 1.4),
    (W/2 - 0.7,    D/2 - 0.7, H - 0.55, lantern_emit_red,    160, 0.18, 1.4),
]
...
for i, off in enumerate([-0.010, 0.010]):
    cyl(f"Chopstick{i}", (bar_x - 0.35, bar_y + 0.20 + off, bar_top_z + 0.02), 0.006, 0.35, bamboo_mat,
...
for i, (dx, dy, m) in enumerate([(-0.48, 0.12, ceramic_w), (-0.25, 0.18, ceramic_g)]):
    cyl(f"BarDish{i}", (bar_x + dx, bar_y + dy, bar_top_z + 0.01), 0.07, 0.015, m)
...
for i in range(5):
    ...
    box(f"NorenS{i}", (x, noren_y - 0.02, noren_z_top - strip_h/2 - 0.05), (0.015, 0.28, strip_h), m,
...
for i in range(3):
    y = 0.1 + i * 0.2
    box(f"NorenS2_{i}", (-W/2 + 0.07, y, H - 0.35 - 0.35), (0.01, 0.16, 0.6), noren_red)
