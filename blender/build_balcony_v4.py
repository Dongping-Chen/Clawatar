...
m_interior = mat("InteriorWall", (0.92, 0.85, 0.72, 1), roughness=0.95)
m_window_frame = mat("WindowFrame", (0.2, 0.17, 0.12, 1), roughness=0.45, metallic=0.1)
m_glass = mat("Glass", (0.65, 0.75, 0.8, 1), roughness=0.08)
m_window_glow = emit_mat("WindowGlow", (1.0, 0.82, 0.5, 1), strength=9.0)
m_door_glow = emit_mat("DoorGlow", (1.0, 0.75, 0.45, 1), strength=3.0)
...
# ── INTERIOR EXTENSION ───────────────────────────────────────────────
# Small interior floor section leading into room
box("InteriorFloor", (0, -2.45, -0.06), (3.4, 1.2, 0.06), m_floor)

# Interior wall behind character (negative Y)
wall_y = -2.9
box("InteriorWall", (0, wall_y, 1.25), (3.6, 0.08, 2.4), m_interior)

# Window frame + warm glow
win_cx, win_cz = -0.7, 1.35
win_w, win_h = 1.6, 1.1
frame = 0.07
frame_depth = 0.06
frame_y = wall_y + 0.06
box("WinTop", (win_cx, frame_y, win_cz + win_h/2), (win_w + frame, frame_depth, frame), m_window_frame)
box("WinBot", (win_cx, frame_y, win_cz - win_h/2), (win_w + frame, frame_depth, frame), m_window_frame)
box("WinL", (win_cx - win_w/2, frame_y, win_cz), (frame, frame_depth, win_h), m_window_frame)
box("WinR", (win_cx + win_w/2, frame_y, win_cz), (frame, frame_depth, win_h), m_window_frame)
box("WinGlow", (win_cx, wall_y - 0.06, win_cz), (win_w - 0.1, 0.03, win_h - 0.1), m_window_glow)

# Sliding door frame + glass glow
door_cx, door_cz = 1.35, 1.05
door_w, door_h = 1.2, 2.1
door_frame = 0.06
box("DoorTop", (door_cx, frame_y, door_cz + door_h/2), (door_w + door_frame, frame_depth, door_frame), m_window_frame)
box("DoorBot", (door_cx, frame_y, door_cz - door_h/2), (door_w + door_frame, frame_depth, door_frame), m_window_frame)
box("DoorL", (door_cx - door_w/2, frame_y, door_cz), (door_frame, frame_depth, door_h), m_window_frame)
box("DoorR", (door_cx + door_w/2, frame_y, door_cz), (door_frame, frame_depth, door_h), m_window_frame)
box("DoorGlass", (door_cx, frame_y + 0.015, door_cz), (door_w - 0.1, 0.02, door_h - 0.12), m_glass)
box("DoorGlow", (door_cx, wall_y - 0.06, door_cz), (door_w - 0.08, 0.03, door_h - 0.1), m_door_glow)
...
# ── POTTED PLANTS ────────────────────────────────────────────────────
def make_plant(name, px, py, pot_h=0.25, pot_r=0.12, tall=False, base_z=0.0):
    cyl(f"{name}Pot", (px, py, base_z + pot_h/2), pot_r, pot_h, m_pot)
    cyl(f"{name}Soil", (px, py, base_z + pot_h-0.01), pot_r-0.01, 0.025, m_soil)
    ...
    for j in range(n_leaves):
        ...
        lz = base_z + pot_h + random.uniform(0.05, max_h)
...
# Side table + plant shelf on interior side
stx, sty = 1.7, -2.1
box("IntTableTop", (stx, sty, 0.45), (0.45, 0.3, 0.04), m_table)
for dx, dy in [(-0.18,-0.12),(0.18,-0.12),(-0.18,0.12),(0.18,0.12)]:
    cyl(f"IntTableLeg{dx}{dy}", (stx+dx, sty+dy, 0.22), 0.018, 0.4, m_table)
make_plant("IntPlant", stx, sty, pot_h=0.16, pot_r=0.08, tall=False, base_z=0.47)
