"""
Swimming Pool Scene v1 — Reze & Denji Pool Scene (Chainsaw Man)
Nighttime school pool with moonlight, underwater glow, romantic mood.
Stage concept: open front+right, pool area with deck.
Cycles renderer for quality lighting.
"""
...
def build_pool_area():
    ...
    m_tile = mat("Tile", (0.15, 0.35, 0.55, 1), 0.3)
    m_tile_light = mat("TileL", (0.20, 0.45, 0.65, 1), 0.3)
    ...
    # ── WATER SURFACE ──
    m_water = glass_mat("Water", (0.10, 0.30, 0.50, 1), 0.5)
...
def build_sky():
    ...
    # Moon
    m_moon = emit_mat("Moon", (0.95, 0.95, 0.88, 1), 8.0)
    sphere("Moon", (2.0, -hd-0.8, 3.5), 0.25, m_moon)

    # Stars
    m_star = emit_mat("Star", (0.95, 0.95, 1.0, 1), 4.0)
    import random
    random.seed(42)
    for i in range(30):
...
def build_lighting():
    ...
    # 1. Moonlight — cool blue, main directional
    ...
    # 2. Pool underwater glow — teal area lights
    for i, (lx, ly) in enumerate([(-1.5, -2.0), (1.5, -2.0), (-1.5, 1.4), (1.5, 1.4)]):
        ...
        p.data.shadow_soft_size = 0.5

    # 3. Deck lights — warm spots (like pool area lamps)
    for i, (lx, ly) in enumerate([(-3.0, 2.0), (3.0, 2.0)]):
        bpy.ops.object.light_add(type='SPOT', location=(lx, ly, 2.0))
        sp = bpy.context.active_object; sp.name = f"DeckL{i}"
        sp.data.energy = 80
        sp.data.color = (1.0, 0.90, 0.70)
        sp.data.spot_size = math.radians(60); sp.data.spot_blend = 0.7
        sp.rotation_euler = (math.radians(90), 0, 0)

    # 4. Front fill — very subtle warm
    ...
    ff.data.size = 4.0; ff.data.size_y = 2.0
...
def render():
    ...
    for name, (loc, rot, lens) in angles.items():
        ...
        scene.camera = co
        scene.render.filepath = os.path.join(OUTPUT_DIR, f"pool_{name}.png")
        bpy.ops.render.render(write_still=True)
        print(f"  ✓ pool_{name}.png")

    g = os.path.join(OUTPUT_DIR, "swimming-pool-v1.glb")
...
def main():
    print("="*50)
    print("  Swimming Pool v1 — Moonlit Night")
    print("="*50)
    clear_scene()
    build_pool_area()
    build_surroundings()
