...
mat_red_neon = emit_mat("RedNeon", (1.0, 0.01, 0.005, 1.0), strength=18.0)
mat_red_neon_strong = emit_mat("RedNeonStrong", (1.0, 0.005, 0.002, 1.0), strength=40.0)
mat_sign_glow = emit_mat("SignGlow", (0.2, 0.9, 0.4, 1.0), strength=5.0)
mat_sign_glow2 = emit_mat("SignGlow2", (1.0, 0.2, 0.5, 1.0), strength=4.0)
mat_window_lit = emit_mat("WindowLit", (1.0, 0.85, 0.5, 1.0), strength=1.5)
mat_vend_glow = emit_mat("VendGlow", (0.3, 0.7, 1.0, 1.0), strength=5.0)
mat_vend_glow2 = emit_mat("VendGlow2", (1.0, 0.4, 0.1, 1.0), strength=4.0)
mat_traffic_red = emit_mat("TrafficRed", (1.0, 0.1, 0.05, 1.0), strength=8.0)
mat_traffic_green = emit_mat("TrafficGreen", (0.1, 1.0, 0.3, 1.0), strength=6.0)
mat_neon_blue = emit_mat("NeonBlue", (0.1, 0.4, 1.0, 1.0), strength=8.0)
mat_neon_pink = emit_mat("NeonPink", (1.0, 0.1, 0.5, 1.0), strength=7.0)
mat_conv_store = emit_mat("ConvStore", (0.1, 1.0, 0.5, 1.0), strength=6.0)
...
for cx, cy in [(bw/2, bd/2), (-bw/2, bd/2), (bw/2, -bd/2), (-bw/2, -bd/2)]:
    box(f"NeonStrip_{cx}_{cy}", (bx+cx, by+cy, bh/2+0.06), (0.015, 0.015, bh*0.8), mat_red_neon)
...
for py_off in [-0.3, 0.0, 0.3]:
    cyl(f"Wire_{py_off}", (0.5, py_off, 2.85), 0.003, 6.0, mat_dark_metal, rot=(0, math.radians(90), 0))
# Cross arm wires
for py_off in [-0.2, 0.2]:
    cyl(f"Wire2_{py_off}", (0.5, py_off, 2.55), 0.003, 5.0, mat_dark_metal, rot=(0, math.radians(90), 0))

# ─── BUILDINGS (backdrop) ──────────────────────────────────
buildings = [
    ("Bldg1", (-2.0, 4.0, 1.5), (1.2, 0.8, 3.0), mat_building),
    ("Bldg2", (-0.6, 4.0, 2.0), (1.0, 0.8, 4.0), mat_building2),
    ("Bldg3", (0.6, 4.0, 1.8), (1.1, 0.8, 3.6), mat_building),
    ("Bldg4", (1.8, 4.0, 1.3), (1.0, 0.8, 2.6), mat_building2),
    ("Bldg5", (3.0, 4.0, 2.2), (1.2, 0.8, 4.4), mat_building),
]
...
# Lit windows
windows = [
    (-0.6, 3.59, 1.8, 0.12, 0.15), (-0.6, 3.59, 2.3, 0.12, 0.15),
    (-0.4, 3.59, 1.5, 0.1, 0.12), (0.6, 3.59, 1.5, 0.12, 0.15),
    (0.6, 3.59, 2.1, 0.12, 0.15), (1.8, 3.59, 1.2, 0.1, 0.12),
    (-2.0, 3.59, 1.6, 0.1, 0.12), (-2.0, 3.59, 2.2, 0.1, 0.12),
    (3.0, 3.59, 2.0, 0.12, 0.15), (3.0, 3.59, 3.0, 0.12, 0.15),
    (-3.49, 0.5, 1.5, 0.12, 0.15), (-3.49, 0.2, 2.0, 0.1, 0.12),
    (-0.8, 3.59, 2.8, 0.1, 0.12), (0.3, 3.59, 2.5, 0.1, 0.12),
    (2.5, 3.59, 2.8, 0.12, 0.15), (4.8, -0.5, 1.5, 0.12, 0.15),
]
...
box("ConvSign", (0.6, 3.58, 2.8), (0.7, 0.04, 0.18), mat_conv_store)
# Bar/restaurant sign
box("BarSign", (1.8, 3.58, 2.0), (0.4, 0.03, 0.12), mat_sign_glow2)
# Additional neon signs
box("KaraokeSign", (-1.5, 3.58, 2.5), (0.35, 0.03, 0.1), mat_neon_pink)
box("DrugStoreSign", (3.0, 3.58, 3.5), (0.5, 0.03, 0.12), mat_neon_blue)
...
box("TLGreen", (tl_x-0.8, tl_y-0.042, 2.28), (0.04, 0.01, 0.04),
    emit_mat("TLGreenDim", (0.05, 0.3, 0.1, 1.0), 0.5))
...
rbox("VendingMachine1", (1.5, 3.0, 0.45), (0.3, 0.25, 0.85), mat_vend, r=0.02)
box("VendLight1", (1.5, 2.75, 0.7), (0.25, 0.02, 0.2), mat_vend_glow)
# Red/orange vending machine
rbox("VendingMachine2", (1.15, 3.0, 0.45), (0.28, 0.25, 0.85), mat_vend2, r=0.02)
box("VendLight2", (1.15, 2.75, 0.7), (0.22, 0.02, 0.2), mat_vend_glow2)
...
box("StreetSign2", (0.1, -1.2, 1.4), (0.2, 0.02, 0.06),
    emit_mat("SignReflect", (0.8, 0.8, 0.8, 1.0), 0.3))

# ─── AWNINGS ──────────────────────────────────────────────
for i, (ax, az, aw) in enumerate([(-0.6, 1.1, 0.5), (0.6, 0.9, 0.4), (1.8, 0.8, 0.35)]):
    box(f"Awning_{i}", (ax, 3.58, az), (aw, 0.15, 0.02), mat_awning if i%2==0 else mat_awning2)

# ─── AC UNITS ─────────────────────────────────────────────
for i, (acx, acz) in enumerate([(-1.6, 1.3), (-0.3, 1.8), (1.0, 1.6), (2.5, 1.4)]):
    rbox(f"AC_{i}", (acx, 3.58, acz), (0.15, 0.1, 0.1), mat_ac, r=0.01)

# ─── PIPES ─────────────────────────────────────────────────
for i, (px, pz, ph) in enumerate([(-1.8, 1.5, 1.5), (0.2, 1.5, 2.0), (2.2, 1.2, 1.8)]):
    cyl(f"Pipe_{i}", (px, 3.58, pz), 0.015, ph, mat_dark_metal)

# ─── SHUTTERS ──────────────────────────────────────────────
box("Shutter1", (-0.6, 3.58, 0.45), (0.8, 0.03, 0.85), mat_shutter)
box("Shutter2", (1.8, 3.58, 0.4), (0.6, 0.03, 0.75), mat_shutter)
...
# ─── TRASH CAN ─────────────────────────────────────────────
cyl("TrashCan", (1.2, 2.9, 0.2), 0.06, 0.4, mat_dark_metal)
...
# ─── COMPOSITING (bloom/glare) ─────────────────────────────
try:
    scene.use_nodes = True
    tree = scene.node_tree
    if tree is None:
        # Blender 5.0 may need compositor workspace
        bpy.ops.node.new_node_tree(type='CompositorNodeTree')
        tree = scene.node_tree
    if tree:
        comp_nodes = tree.nodes
        comp_links = tree.links
        for n in list(comp_nodes): comp_nodes.remove(n)
        ...
        comp_links.new(glare.outputs["Image"], comp_out.inputs["Image"])
except Exception as e:
    print(f"Compositor setup skipped: {e}")

# ─── SKIP RENDERS — just export GLB ────────────────────────
# (Renders take too long with volume scatter + Cycles)

# ─── EXPORT GLB ────────────────────────────────────────────
glb_path = os.path.join(OUT, "phone-booth-v3.glb")
