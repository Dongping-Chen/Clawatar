...
m_bulb = emit_mat("Bulb", (1.0, 0.85, 0.5, 1), strength=20.0)
...
# Massive emissive panels behind scene, VERY bright, layered
# Camera sees these ABOVE the buildings
sky_layers = [
    # z, color_rgba, strength, height
    (-2.5, (1.0, 0.5, 0.05, 1), 18.0, 3.0),   # deep golden orange (horizon)
    (-0.5, (1.0, 0.6, 0.1, 1), 15.0, 2.5),     # bright orange
    (1.5,  (1.0, 0.4, 0.12, 1), 12.0, 2.5),    # orange-red
    (3.5,  (0.95, 0.3, 0.2, 1), 9.0, 2.5),     # coral pink
    (5.5,  (0.85, 0.2, 0.35, 1), 7.0, 2.5),    # hot pink
    (7.5,  (0.6, 0.15, 0.5, 1), 5.0, 3.0),     # magenta purple
    (10.0, (0.3, 0.1, 0.55, 1), 3.5, 3.5),     # deep purple
    (13.0, (0.12, 0.06, 0.3, 1), 2.0, 4.0),    # night purple
]
for i, (z, col, st, h) in enumerate(sky_layers):
    m = emit_mat(f"Sky{i}", col, st)
    box(f"Sky{i}", (0, 15, z), (30, 0.1, h), m)

# Sun disc — bright and large at horizon
m_sun = emit_mat("Sun", (1.0, 0.75, 0.2, 1), 50.0)
sphere("Sun", (1.0, 14, -1.0), 1.2, m_sun)
# Sun halo
m_halo = emit_mat("Halo", (1.0, 0.6, 0.15, 1), 8.0)
...
# ── RAILING (front + sides, open to sunset) ─────────────────────────
ry = 1.5; rh = 1.0
# Top rail
...
# ── CITY SKYLINE (lower, so sky gradient is visible above) ───────────
bldgs = [
...
def make_chair(prefix, cx, cy, angle=0):
    ...
    bdx = -0.18*math.sin(a)
    bdy = -0.18*math.cos(a)
    box(f"{prefix}Back", (cx+bdx*0, cy-0.18*math.cos(a), 0.7), (0.38, 0.02, 0.5), m_chair, rot=rot)
    # Back bars
    for off in [-0.12, 0, 0.12]:
        bx2 = cx + off*math.cos(a)
        by2 = cy - 0.18*math.cos(a) + off*math.sin(a)*0  # simplified
        cyl(f"{prefix}Bar{off}", (bx2, cy - 0.18, 0.7), 0.008, 0.48, m_chair, rot=rot)

make_chair("ChA", -0.3, -0.75, 0)
make_chair("ChB", 0.5, -0.3, 25)
...
# Big warm area fill from the sky direction
...
# Secondary warm fill (lower, from horizon)
...
if bg:
    bg.inputs["Color"].default_value = (0.01, 0.005, 0.02, 1)
    bg.inputs["Strength"].default_value = 0.1

# ── CAMERAS & RENDER ────────────────────────────────────────────────
cams = {
    "main": ((0.3, -3.2, 1.6), (math.radians(74), 0, math.radians(3)), 32),
    "skyline": ((-0.3, -0.5, 1.1), (math.radians(82), 0, math.radians(-8)), 28),
    "closeup": ((0.3, -1.2, 0.95), (math.radians(78), 0, math.radians(10)), 40),
    "wide": ((0.2, -4.5, 2.2), (math.radians(70), 0, math.radians(2)), 28),
}

for name, (loc, rot, lens) in cams.items():
    bpy.ops.object.camera_add(location=loc)
    cam = bpy.context.active_object; cam.name = f"Cam_{name}"
    cam.rotation_euler = rot; cam.data.lens = lens
    scene.camera = cam
