"""
v7 — Warmer + Cozier Bedroom (GLB export only)
Key fixes:
- Warm emissive string lights + desk lamp glow
- Slightly warmer wall tints
- Cozy rug + accent glow items
- Clear center floor, furniture moved to edges
"""
...
def build_all():
    ...
    m_ac = mat("Accent", (1.0, 0.94, 0.96, 1), 0.8)  # warmer pink wall
    ...
    rbox("Rug", (0.1,0.35,0.006), (1.4,0.95,0.006), m_rg, 0.02)
    ...
    # Fairy lights — warm, visible emission
    m_fb = emit_mat("FB", (1.0,0.82,0.50,1), 18.0)
    m_fp = emit_mat("FP", (1.0,0.82,0.50,1), 16.0)
    for i in range(20):
...
def main():
    print("="*50)
    print("  v7 — Warmer + Cozier Bedroom")
    print("="*50)
    clear_scene()
    build_all()
    build_lighting()
