"""
Swimming Pool v4 — NIGHT POOL PARTY (BRIGHTER)
Fixes: brighter, more atmospheric night pool scene.

Key changes:
- Stronger emissive/glow elements (pool, orbs, moon, stars)
- Extra warm point lights for lounge seating
- Clear center stage for character
- GLB export only
"""
...
def build_pool_area():
    ...
    # ═══ WATER — glowing surface (emissive turquoise) ═══
    m_water = emit_mat("WaterGlow", (0.08, 0.75, 0.85, 1), 10.0)
...
def build_surroundings():
    ...
    for i, (cx, cy, rot) in enumerate([
        (3.6, 1.5, 0), (3.6, -0.7, 0), (-3.6, 0.7, math.radians(180))
    ]):
        # Frame
    ...
    # ── PARTY ORB LIGHTS (emissive spheres) ──
    orb_colors = [
        (0.95, 0.25, 0.45, 1),  # pink
        (0.25, 0.85, 0.95, 1),  # cyan
        (0.75, 0.40, 1.00, 1),  # purple
        (1.00, 0.70, 0.20, 1),  # warm amber
    ]
    orb_positions = [
        (-2.8, -1.9, 1.1),
        (2.8, -1.9, 1.1),
    ...
    for i, (ox, oy, oz) in enumerate(orb_positions):
        m_orb = emit_mat(f"PartyOrbMat{i}", orb_colors[i % len(orb_colors)], 35.0)
    ...
    rbox("Tw1", (3.6, 1.9, 0.22), (0.35, 0.15, 0.02), m_tw1, 0.008)
    rbox("Tw2", (-3.6, 0.9, 0.22), (0.30, 0.12, 0.02), m_tw2, 0.008)

    # ── FLIP FLOPS ──
    m_ff = mat("FlipF", (0.95, 0.60, 0.65, 1), 0.8)
    rbox("FF1", (3.2, 2.1, 0.015), (0.06, 0.14, 0.02), m_ff, 0.008, rot=(0,0,math.radians(10)))
    rbox("FF2", (3.32, 2.08, 0.015), (0.06, 0.14, 0.02), m_ff, 0.008, rot=(0,0,math.radians(5)))
...
def build_lighting():
    ...
    # 2. UNDERWATER POOL LIGHTS — brighter glow
    ...
    # 3. PARTY ORB LIGHTS — main hero lights (very bright)
    for i, (lx, ly, lz) in enumerate([
        ...
        o = bpy.context.active_object; o.name = f"PartyOrbLight{i}"
        o.data.energy = 40
        o.data.color = (1.0, 0.85, 0.95)
        o.data.shadow_soft_size = 1.2

    # 4. Lounge warm fills — extra seating illumination
    for i, (lx, ly, lz) in enumerate([
        (3.6, 1.5, 1.3),
        (3.6, -0.7, 1.3),
        (-3.6, 0.7, 1.3),
        ...
        bpy.ops.object.light_add(type='POINT', location=(lx, ly, lz))
        w = bpy.context.active_object; w.name = f"LoungeFill{i}"
        w.data.energy = 18
        w.data.color = (1.0, 0.92, 0.78)
        w.data.shadow_soft_size = 1.0

    # 5. Water bounce — subtle teal, reduced
...
def main():
    print("="*50)
    print("  Swimming Pool v4 — NIGHT POOL PARTY (BRIGHTER)")
    print("="*50)
    clear_scene()
    build_pool_area()
    build_surroundings()
