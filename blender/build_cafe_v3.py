"""
Café Scene v3 — Reze's Coffee Shop
v1 issues: no visible coffee machine/coffee, too empty, pendant lights not bright enough
v3 fixes: BIGGER coffee machine, coffee cups on tables, more items, warmer lighting
"""
...
def build_decorations():
    ...
    m_wg = emit_mat("WinG", (0.20, 0.25, 0.35, 1), 1.0)  # night — dim blue
    ...
    m_sl = emit_mat("SLight", (1.0, 0.92, 0.70, 1), 10.0)
    m_slp = emit_mat("SLightP", (1.0, 0.80, 0.85, 1), 8.0)
    for i in range(14):
        sx = -hw+0.4 + i*(STAGE_W-0.8)/13
        sz = STAGE_H-0.15 + math.sin(i*0.8)*0.04
        m = m_sl if i%3!=0 else m_slp
        sphere(f"SL{i}", (sx, -hd+0.08, sz), 0.015, m)
...
def build_lighting():
    ...
    s.data.energy = 0.3  # MINIMAL — pendants are the star
    ...
    if bg:
        ...
        bg.inputs["Strength"].default_value = 0.05  # near black — light sources only
...
def render():
    ...
    for name, (loc, rot, lens) in angles.items():
        ...
        scene.camera = co
        scene.render.filepath = os.path.join(OUTPUT_DIR, f"cafe3_{name}.png")
        bpy.ops.render.render(write_still=True)
        print(f"  ✓ cafe3_{name}.png")

    g = os.path.join(OUTPUT_DIR, "cafe-v3.glb")
...
def main():
    print("="*50)
    print("  Café v3 — Reze's Coffee Shop")
    print("="*50)
    clear_scene()
    build_shell()
    build_counter()
