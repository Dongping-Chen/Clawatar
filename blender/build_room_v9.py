"""
v9 — Bedroom furniture color + detail pass (GLB export only)
Key fixes:
- Emissive wood tones for desk/bed/shelves (>=3.0)
- Bigger laptop + desk lamp glow + mug/books
- Bed pillows + plushie
- Wall art with visible colored inserts + extra frames
- Curtain tweaks + added desk rug
- Bookshelf books colored
- String lights with small warm glow bulbs
"""
...
def build_all():
    ...
    # Subtle wall panels for variation
    m_wp = mat("WallPanel", (0.98, 0.93, 0.95, 1), 0.85)
    for i,xx in enumerate([-1.6,-0.6,0.5,1.4]):
        rbox(f"WP{i}", (xx,-hd+0.02,1.4), (0.50,0.02,0.90), m_wp, 0.01)
    ...
    m_bf = emit_mat("BedF", (0.46, 0.30, 0.20, 1), 4.5)
    m_mt = mat("Matt", (0.98, 0.96, 0.94, 1), 0.8)
    m_bl = mat("Blanket", (1.0, 0.55, 0.70, 1), 0.85)
    m_bl2 = mat("BlFold", (0.98, 0.50, 0.66, 1), 0.85)
    ...
    # Cylindrical pillow + plushie
    m_pcyl = mat("PillowCyl", (1.0, 0.92, 0.92, 1), 0.8)
    cyl("PillowCyl", (bx-0.05,by-0.20,0.50), 0.08, 0.35, m_pcyl, rot=(math.radians(90),0,0))
    m_plush = mat("Plush", (0.86, 0.78, 0.95, 1), 0.9)
    sphere("Plush", (bx+0.32,by+0.10,0.52), 0.10, m_plush, (1.1,0.9,0.8))
    ...
    # ── DESK ──
    m_dt = emit_mat("DT", (0.78, 0.62, 0.42, 1), 3.5)
    m_dl = emit_mat("DL", (0.70, 0.55, 0.40, 1), 3.5)
    dx, dy = hw-1.0, -hd+0.4
    ...
    m_cs = emit_mat("CS", (0.95, 0.74, 0.82, 1), 3.2)
    ...
    glb("laptop.glb", loc=(dx-0.18,dy-0.02,0.75), sc=2.6)
    glb("books.glb", loc=(dx+0.35,dy-0.05,0.74), sc=2.0)
    glb("books.glb", loc=(dx+0.10,dy-0.18,0.74), sc=1.6)
    ...
    # Desk rug
    m_dr = emit_mat("DeskRug", (0.60, 0.74, 0.92, 1), 3.0)
    rbox("DeskRug", (dx,dy+0.15,0.006), (1.25,0.85,0.006), m_dr, 0.02)

    # ── BOOKSHELF ──
    sx, sy = -hw+0.20, -0.3
    m_sh = emit_mat("Sh", (0.62, 0.48, 0.32, 1), 4.0)
    ...
    for si,sz in enumerate([0.02,0.40,0.77]):
        for j in range(6):
            ...
            rbox(f"Bk{si}{j}", (sx,sy-sw/2+0.06+j*0.10,sz+0.14), (0.025+(j%3)*0.008, 0.17, 0.22+(j%4)*0.04), emit_mat(f"B{si}{j}",c,3.2), 0.004)
    ...
    # ── WALL DECOR ──
    m_fr = emit_mat("PF", (0.78,0.68,0.58,1), 3.0)
    for i,(loc,dim,col) in enumerate([
        ...
        box(f"Pi{i}", (loc[0],loc[1]+0.015,loc[2]), (dim[0]-0.06,0.01,dim[2]-0.06), emit_mat(f"P{i}",col,3.2))
    # Extra frames
    for j,(loc,dim,col) in enumerate([
        ((0.75,-hd+0.04,2.05),(0.26,0.02,0.36),(0.75,0.55,0.95,1)),
        ((1.05,-hd+0.04,1.6),(0.32,0.02,0.24),(0.95,0.70,0.55,1))]):
        rbox(f"FrX{j}", loc, dim, m_fr, 0.008)
        box(f"PiX{j}", (loc[0],loc[1]+0.015,loc[2]), (dim[0]-0.06,0.01,dim[2]-0.06), emit_mat(f"PX{j}",col,3.2))
...
# ═══════ EXPORT ═══════
def render():
    g = os.path.join(OUTPUT_DIR, "cozy-bedroom-v9.glb")
...
def main():
    print("="*50)
    print("  v9 — Bedroom furniture detail pass")
    print("="*50)
    clear_scene()
    build_all()
    build_lighting()
